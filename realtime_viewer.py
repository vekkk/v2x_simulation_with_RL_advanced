...
class Viewer:
    def __init__(self, n_cars=12, use_drl=True, fps=30):
        pygame.init()
        pygame.display.set_caption("Vehicular DRL — Advanced Real-Time Viewer")
        ...
        if use_drl:
            ...
            self.agent = DQNAgent(self.cfg.drl, action_dim=16, device=device)
            model_path = os.path.join("data", "models", "dqn_final.pt")
            if os.path.exists(model_path):
                self.agent.load(model_path)
                print(f"[Viewer] Loaded trained DRL model: {model_path}")
            else:
    ...
    def chooser(self, sim, state_vec, avail_mask):
        if self.use_drl:
            # Convert 4-action availability to 16-action availability
            actions = build_actions()
            action_mask = action_valid_mask(actions, avail_mask)
            return int(self.agent.act(state_vec, action_mask, greedy=True))
    ...
    def draw_hud(self, fps):
        lines = [
            f"FPS: {fps:.0f}  x{self.speed_mult}   Mode: {'DRL-DDQN' if self.use_drl else 'Heuristic'}",
            "Keys: SPACE pause | +/- speed | D toggle DRL | R reset | Q quit",
            f"Vehicles: {len(self.sim.vehicles)} | Actions: 16 (Single+Redundant+Division)",
        ]
        for i, txt in enumerate(lines):
            self.screen.blit(self.font.render(txt, True, WHITE), (10, 10 + 18*i))
    ...
    def draw_cars(self):
        ...
        for (x, y, action_idx, info) in self.sim.get_positions():
            # Get action info for coloring
            actions = build_actions()
            if action_idx < len(actions):
                ...
                else:  # redundant or division
                    # Use a mixed color for dual-RAT actions
                    rat1, rat2 = action[1], action[2]
                    color = tuple((np.array(RAT_COLORS[rat1]) + np.array(RAT_COLORS[rat2])) // 2)
            else:
                color = GRAY  # fallback
