...
export class NetworkManager {
    ...
    calculateSNRBasedPacketLoss(snr, networkType) {
        ...
        if (snr >= minSNR + 15) {
            // Excellent SNR - minimal packet loss but not zero
            return basePacketLoss * 0.2;
        } else if (snr >= minSNR + 10) {
            // Very good SNR - low packet loss
            return basePacketLoss * 0.5;
        } else if (snr >= minSNR + 5) {
            // Good SNR - moderate packet loss
            return basePacketLoss * 0.8;
        } else if (snr >= minSNR) {
            // Acceptable SNR - base packet loss
            return basePacketLoss;
        } else if (snr >= minSNR - 5) {
            // Poor SNR - increased packet loss
            return Math.min(0.4, basePacketLoss * 2.5);
        } else if (snr >= minSNR - 10) {
            // Very poor SNR - high packet loss
            return Math.min(0.6, basePacketLoss * 5);
        } else {
            // Extremely poor SNR - very high packet loss
    ...
    update(deltaTime, currentTime) {
        ...
        this.vehicleManager.vehicles.forEach(vehicle => {
            ...
            if (currentTime - lastPacketTime >= sendInterval) {
                ...
                let packetLossRate = commPath.packetLossRate || 0.1;
                
                // Add environmental factors for more realistic packet loss
                const environmentalLoss = this.calculateEnvironmentalPacketLoss(vehicle, selectedNetwork, messageType);
                packetLossRate = Math.min(0.9, packetLossRate + environmentalLoss);
                ...
                // Enhanced logging with packet loss details
                if (Math.random() < 0.1) { // Log 10% of packets to avoid spam
                    console.log(`Vehicle ${vehicle.userData.id}: ${messageType} via ${selectedNetwork} (${commPath.type}) - SNR: ${commPath.snr?.toFixed(1)}dB, Base Loss: ${(commPath.packetLossRate * 100).toFixed(1)}%, Env Loss: ${(environmentalLoss * 100).toFixed(1)}%, Total Loss: ${(packetLossRate * 100).toFixed(1)}%, Success: ${packetSuccess}, Distance: ${commPath.distance.toFixed(1)}m`);
                }
            }
        });
    ...
    calculateEnvironmentalPacketLoss(vehicle, networkType, messageType) {
        let environmentalLoss = 0;
        
        // Network congestion factor (more vehicles = more congestion)
        const totalVehicles = this.vehicleManager.vehicles.length;
        const congestionFactor = Math.min(0.05, (totalVehicles - 10) * 0.002); // Increases with vehicle count
        environmentalLoss += Math.max(0, congestionFactor);
        
        // Random interference (simulates real-world RF interference)
        const interferenceFactor = Math.random() * 0.03; // 0-3% random interference
        environmentalLoss += interferenceFactor;
        
        // Distance-based degradation (farther = more loss)
        const distance = vehicle.userData.communicationAnalytics?.selectedOption?.distance || 50;
        const distanceFactor = Math.min(0.04, distance / 1000); // Increases with distance
        environmentalLoss += distanceFactor;
        
        // Network-specific factors
        switch(networkType) {
            case 'DSRC':
                // DSRC suffers from vehicle density
                environmentalLoss += Math.min(0.02, totalVehicles * 0.0005);
                break;
            case 'WIFI':
                // WiFi suffers from interference
                environmentalLoss += Math.random() * 0.025;
                break;
            case 'LTE':
                // LTE suffers from cell tower load
                environmentalLoss += Math.random() * 0.02;
                break;
        }
        
        // Message type priority affects loss (higher priority = better protection)
        const messageConfig = CONFIG.MESSAGE_TYPES[messageType];
        const priorityFactor = (5 - messageConfig.priority) * 0.005; // Lower priority = more loss
